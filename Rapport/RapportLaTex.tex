...
\subsubsection{R1}
	...
	$c \leftarrow \sigma_{etablissementID \ IN (b) \wedge \#Etablissement >= 3 }$
...
\subsubsection{R2}
	...
	$g \leftarrow \pi_{Nom}(f \bowtie_{clientID=ID} Utilisateur)$

\subsubsection{R3}
	$a \leftarrow \pi_{etablissementID}(Etablissement)$

	$b \leftarrow \sigma_{etablissemntID \ IN \ (a) \wedge \#Etablissement \ <= \ 1}(Commentaire)$
...
\centerline{\includegraphics[scale=0.3]{acceuil.png}}
...
Pour la démonstration, on va se déconnecter et se connecter avec un autre utilisateur qui sera admin pour pouvoir montrer quelque fonctionnalité
