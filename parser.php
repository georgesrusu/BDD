<?php
    ...
    foreach ($listCafe as $cafe) {
		 ...
		 */
		try{
			$sql = 'SELECT ID FROM Etablissement WHERE nom="'.$cafeName.'"';
			$stmt = $conn->prepare($sql); 
			$stmt->execute();
			$result=$stmt->fetch();
			if ($result==""){
				echo "ETABLISSEMENT DOESN'T EXIST <br/>";
				$sql = 'INSERT INTO Etablissement (nom,rue,numero,codePostal,localite,longitude,latitude,telephone,lienWeb,type) VALUES ("'.$cafeName.'","'.$street.'","'.(int)$num.'","'.(int)$zip.'","'.$city.'","'.(float)$longitude.'","'.(float)$latitude.'","'.$tel.'","'.$site.'","Bar")';
    			$conn->exec($sql);
				echo "INSERT ETABLISSEMENT SUCCESS <br/>";
				$sql = 'SELECT ID FROM Etablissement WHERE nom="'.$cafeName.'"';
				$stmt = $conn->prepare($sql); 
